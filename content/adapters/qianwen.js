...
window.GEO.QianwenAdapter.prototype.extract = function() {
  ...
  for (var i = 0; i < rounds.length; i++) {
    ...
    // 优先从 round 内部的 script 标签直接提取引用
    var citations = this._parseCitationsFromRound(round);
...
/**
 * 从单个 .chat-round 内部的 script 标签解析引用数据
 * 千问的 script[type="application/json"] 嵌套在 .chat-round 内部
 */
window.GEO.QianwenAdapter.prototype._parseCitationsFromRound = function(round) {
  var scripts = round.querySelectorAll('script[type="application/json"]');
  var allCitations = [];
  var seen = {};
  ...
  for (var i = 0; i < scripts.length; i++) {
    ...
    try {
      ...
      // 路径1: data.data.originalData.content
      if (data.data && data.data.originalData && data.data.originalData.content) {
        content = data.data.originalData.content;
      }
      // 路径2: data.originalData.content (无嵌套 data)
      else if (data.originalData && data.originalData.content) {
