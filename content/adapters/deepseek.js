...
/**
 * 从 AI 回复块提取引用链接
 * 引用来源卡片: div._02fb570._9887d4d > a._04ab7b1
 * 也可能聚合在 .f2021e64 容器内
 */
window.GEO.DeepSeekAdapter.prototype._extractCitationsFromBlock = function(msg) {
  var citations = [];

  // 主策略: 直接找所有引用卡片链接 a._04ab7b1
  var refLinks = msg.querySelectorAll('a[class*="_04ab7b1"]');

  // 备选策略: .f2021e64 容器内所有链接
  ...
  for (var i = 0; i < refLinks.length; i++) {
    ...
    // 引用编号链接（如 "-15", "-24"）用域名作为标题
    if (/^-?\d+$/.test(title)) {
      title = this._getDomain(href);
    }
  ...
  // 先收集所有卡片引用，再补充行内引用（不在卡片中的）
  var allLinks = msg.querySelectorAll('a');
  for (var k = 0; k < allLinks.length; k++) {
    var aLink = allLinks[k];
    var aHref = aLink.href;
    if (!aHref || aHref.indexOf('deepseek.com') !== -1) continue;
    // 跳过已经被 refLinks 覆盖的
    var cls = aLink.className || '';
    if (cls.indexOf('_04ab7b1') !== -1) continue;

    var aTitle = aLink.textContent.trim();
    if (/^-?\d+$/.test(aTitle)) {
      aTitle = this._getDomain(aHref);
    }
    if (!aTitle) continue;
